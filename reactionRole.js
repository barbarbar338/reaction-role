...
class ReactionRole {
  ...
  init() {
    ...
    this.client.on("messageReactionAdd", (messageReaction, user) => {
			...
			(async () => {
				for (var { channel, reactions, disjoint } of this.config) {
		...
		this.client.on("messageReactionRemove", (messageReaction, user) => {
			...
			var emojiDiscriminator = this.getEmojiDiscriminator(messageReaction.emoji);
			(async () => {
				for (var { disjoint, channel, reactions } of this.config) {
  ...
  createMessage() {
    let reactions = [];
    for (var i = 3; i < arguments.length; i++) reactions.push(arguments[i]);
    this.config.push({
        "message": arguments[0],
        "channel": arguments[1],
        "disjoint": arguments[2],
        "reactions": reactions
      })
  }
